...
class TopVotedCandidate {
	List<List<Vote>> A;
	public TopVotedCandidate(int[] persons, int[] times) {//参数为每个时间被投票的人的代号
		A = new ArrayList<>();
		Map<Integer, Integer> count = new HashMap<>();
		
		//int max = persons[0];
		for (int i = 0; i < persons.length; ++i) {
			int p = persons[i], t = times[i];//投票代号及投票的时间
			int c = count.getOrDefault(p, 0)+1;//如果有p就返回p的值没有的话就返回1
			count.put(p,c);
			while (A.size() <= c)
