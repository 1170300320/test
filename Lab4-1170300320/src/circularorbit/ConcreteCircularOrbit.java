...
public class ConcreteCircularOrbit<L,E> implements CircularOrbit<L,E> {
	...
	public boolean addtrack(Track<E> track) {
		tracks.add(track);
		logging.logFunction("操作：添加一个轨道");
		return true;
	}

	@Override
	public boolean removetrack(int num) {
		logging.logFunction("操作：删除编号为"+num+"的轨道");
	...
	public void addcenter(L object) {
	   logging.logFunction("操作：添加一个中心物体");
	   this.center = object;
	}
	...
	public boolean addtotrack(E obj1, int num) {
		...
		logging.logFunction("操作：向"+num+"号轨道添加一个物体");
		try {
            
            PrintStream ps = new PrintStream(new FileOutputStream(file));
            ps.append("操作：向"+num+"号轨道添加一个物体");// 往文件里写入字符串
            ps.close();
        } catch (FileNotFoundException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
